    ...
    <section class="breadcrumbs-custom">
        <div class="parallax-container" data-parallax-img="{{ asset('images/bg-blog-2.jpg') }}">
            ...
            <div class="breadcrumbs-custom-body parallax-content context-dark">
                <div class="container">
                    <h2 class="breadcrumbs-custom-title">{{ $category->name ?? '' }} Collection</h2>
                </div>
            </div>
        </div>
                ...
                <div class="col-lg-4 col-xl-3">
                    <div class="aside row row-30 row-md-50 justify-content-md-between">
                        <div class="aside-item col-sm-6 col-md-5 col-lg-12">
                            <h6 class="aside-title">Sub Categories</h6>
                            <ul class="list-shop-filter">
                                <li>
                                    ...
                                    </label><span class="list-shop-filter-number">({{ $products->count() ?? 0 }})</span>
                                </li>
                                @if ($category->subCategories->count())
                                    @foreach ($category->subCategories as $idx => $subCategpry)
                                        @if ($subCategpry->products->count())
                                            <li>
                                                <label class="checkbox-inline">
                                                    <input name="input-group-radio" value="{{ $subCategpry->id }}"
                                                        type="checkbox" class="category-filter"><span
                                                        class=""></span>{{ $subCategpry->name ?? '' }}
                                                </label><span
                                                    class="list-shop-filter-number">({{ $subCategpry->products ? $subCategpry->products->count() : 0 }})</span>
                                            </li>
                                        @endif
                                    @endforeach
                                @endif
